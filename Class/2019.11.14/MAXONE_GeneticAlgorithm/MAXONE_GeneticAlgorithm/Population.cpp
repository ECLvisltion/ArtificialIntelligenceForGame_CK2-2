...
Population::Population(int pSize, int gSize, double cRate, double mRate)
	: popSize(pSize), chromSize(gSize), crossoverRate(cRate), mutationRate(mRate)
{
	...
	{
		population[i].ResetChromosome(gSize);
	}
...
void Population::Mutation(Chromosome & gene)
{
	...
	int mutationPoint = rand() % chromSize;
	vector<int> g = gene.GetChromosome();
	g[mutationPoint] = 1 - g[mutationPoint];
